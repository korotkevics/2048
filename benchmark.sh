...
CLIENT_ID="llm-test-1"
API_BASE="http://localhost:8085/api"

echo "Starting benchmark for Client ID: $CLIENT_ID"

# 0. Count moves before starting (to avoid including old moves from logs)
MOVES_BEFORE=$(docker compose logs play2048-app 2>/dev/null | grep "\[AI\] Model: qwen2.5:1.5b chosen move:" | wc -l | xargs)
...
MOVES_AFTER=$(docker compose logs play2048-app 2>/dev/null | grep "\[AI\] Model: qwen2.5:1.5b chosen move:" | wc -l | xargs)
...
if [ "$MOVES" -gt 0 ]; then
    # Use awk for division to avoid bc dependency if not present, but bc is common.
